...
class App(ctk.CTk):
    ...
    def criar_sidebar(self):
        ...
        # ── Logo compacto ────────────────────────────────────────────────────
        logo_frame = ctk.CTkFrame(self.sidebar, fg_color=C["sidebar"], corner_radius=0)
        logo_frame.pack(fill="x", pady=(18, 8))

        if os.path.exists(self.logo_path):
            with Image.open(self.logo_path) as logo_image:
                imagem = ctk.CTkImage(light_image=logo_image.copy(),
                                      dark_image=logo_image.copy(), size=(120, 74))
            ctk.CTkLabel(logo_frame, image=imagem, text="").pack(pady=(0, 4))
        else:
            ctk.CTkLabel(logo_frame, text="CW", font=(ff, 34, "bold"),
                         text_color=C["brand"]).pack(pady=(0, 4))

        ctk.CTkLabel(logo_frame, text="CW TRANSPORTADORA",
                     font=(ff, 11, "bold"), text_color=C["text"]).pack()
        ctk.CTkLabel(logo_frame, text="v7 — Painel executivo",
                     font=(ff, 9), text_color=C["text_soft"]).pack(pady=(0, 6))

        # ── Área scrollável ──────────────────────────────────────────────────
        self.sidebar_scroll = ctk.CTkScrollableFrame(
            self.sidebar, fg_color=C["sidebar"],
            scrollbar_button_color=C["surface"],
            scrollbar_button_hover_color=C["surface_2"],
        )
        self.sidebar_scroll.pack(fill="both", expand=True, padx=0, pady=0)

        def secao(titulo):
            ctk.CTkLabel(self.sidebar_scroll, text=titulo.upper(),
                         font=(ff, 9, "bold"), text_color=C["text_soft"]
                         ).pack(anchor="w", padx=22, pady=(14, 4))

        secao("Principal")
        self.criar_botao_menu("🏠", "Página Inicial", self.mostrar_dashboard, "dashboard", C["brand"])

        secao("Operacional")
        self.criar_botao_menu("🧾", "Nova Operação",   self.mostrar_operacoes,       "operacoes",       C["violet"])
        self.criar_botao_menu("📋", "Notas Importadas", self.mostrar_notas,          "notas",           C["violet"])
        self.criar_botao_menu("🚚", "Criar Viagem",     self.mostrar_criar_viagem,   "criar_viagem",    C["sky"])
        self.criar_botao_menu("📊", "Viagens",          self.mostrar_historico,      "historico",       C["sky"])
        self.criar_botao_menu("🏆", "Ranking Clientes", self.mostrar_ranking_clientes,"ranking_clientes",C["amber"])

        secao("Frota")
        self.criar_botao_menu("⛽", "Combustível",  self.mostrar_combustivel, "combustivel", C["emerald"])
        self.criar_botao_menu("🔧", "Manutenção",   self.mostrar_manutencao,  "manutencao",  C["amber"])

        secao("Financeiro")
        self.criar_botao_menu("💳", "Contas",       self.mostrar_contas,       "contas",     C["rose"])
        self.criar_botao_menu("📊", "Relatórios",   self.mostrar_relatorios,   "relatorios", C["sky"])

        secao("RH")
        self.criar_botao_menu("👥", "Funcionários", self.mostrar_funcionarios, "funcionarios", C["cyan"])

        secao("Sistema")
        self.criar_botao_menu("⚙️", "Configurações", self.mostrar_configuracoes, "configuracoes", C["text_muted"])
        self.criar_botao_menu("🔑", "Minha Conta",   self.mostrar_alterar_senha, "minha_conta",   C["text_muted"])

        if auth_service.eh_mestre:
            secao("Administração")
            self.criar_botao_menu("👑", "Usuários",           self.mostrar_usuarios,           "usuarios",           C["brand"])
            self.criar_botao_menu("📋", "Auditoria",          self.mostrar_auditoria,          "auditoria",          C["amber"])
            self.criar_botao_menu("🔄", "Histórico Versões",  self.mostrar_historico_versoes,  "historico_versoes",  C["sky"])
            self.criar_botao_menu("📤", "Publicar Versão",    self.mostrar_publicar_versao,    "publicar_versao",    C["emerald"])
            self.criar_botao_menu("⚙️", "Admin Atualizações", self.mostrar_admin_atualizacoes, "admin_atualizacoes", C["violet"])

        self.after(2000, self._atualizar_badge_contas)

        # ── Base: sync + logout + fechar ─────────────────────────────────────
        self.botao_sync = ctk.CTkButton(
            self.sidebar, text="☁  Sincronizar Nuvem", height=40,
            fg_color=C["surface"], hover_color=C["surface_2"], text_color=C["text"],
            corner_radius=10, anchor="w", font=(ff, 12, "bold"),
            command=lambda: self.sincronizar_nuvem(mostrar_mensagem=True),
        )
        self.botao_sync.pack(fill="x", padx=12, pady=(10, 4))

        ctk.CTkButton(
            self.sidebar, text="🧹  Zerar Distribuição", height=36,
            fg_color=C["surface"], hover_color=C["brand_soft"], text_color=C["brand"],
            corner_radius=10, anchor="w", font=(ff, 11, "bold"),
            command=self.preparar_distribuicao,
        ).pack(fill="x", padx=12, pady=(0, 6))

        # Mini card de status
        self.card_sync = ctk.CTkFrame(self.sidebar, fg_color=C["surface"], corner_radius=10)
        self.card_sync.pack(fill="x", padx=12, pady=(4, 8))

        ctk.CTkLabel(self.card_sync, text="STATUS DA NUVEM",
                     font=(ff, 9, "bold"), text_color=C["text_soft"]
                     ).pack(anchor="w", padx=12, pady=(10, 2))
        self.label_sync_status = ctk.CTkLabel(
            self.card_sync, text="🟡 Verificando",
            font=(ff, 11, "bold"), text_color=C["warning"])
        self.label_sync_status.pack(anchor="w", padx=12)
        self.label_status_ultima = ctk.CTkLabel(
            self.card_sync, text="Ainda não sincronizado",
            font=(ff, 9), text_color=C["text_muted"], wraplength=200, justify="left")
        self.label_status_ultima.pack(anchor="w", padx=12, pady=(2, 2))
        self.label_status_pendencias = ctk.CTkLabel(
            self.card_sync, text="Pendências: 0",
            font=(ff, 9, "bold"), text_color=C["text_muted"])
        self.label_status_pendencias.pack(anchor="w", padx=12)
        self.label_status_resumo = ctk.CTkLabel(
            self.card_sync,
            text=f"Auto a cada {settings.intervalo_sync_segundos}s",
            font=(ff, 9), text_color=C["text_soft"], wraplength=200, justify="left")
        self.label_status_resumo.pack(anchor="w", padx=12, pady=(0, 10))

        ctk.CTkButton(
            self.sidebar, text="↩  Sair da Conta", height=36,
            fg_color=C["surface"], hover_color=C["surface_2"], text_color=C["text"],
            corner_radius=10, anchor="w", font=(ff, 11, "bold"),
            command=self.fazer_logout,
        ).pack(fill="x", padx=12, pady=(2, 4), side="bottom")

        ctk.CTkButton(
            self.sidebar, text="✕  Fechar Sistema", height=36,
            fg_color="transparent", hover_color=C["brand_soft"], text_color=C["text_muted"],
            corner_radius=10, anchor="w", font=(ff, 11, "bold"),
            command=self.fechar_sistema,
        ).pack(fill="x", padx=12, pady=(0, 10), side="bottom")
    ...
    def criar_botao_menu(self, icone, texto, comando, chave_tela, cor_icone):
        """Nova assinatura: (icone, texto, comando, chave, cor_do_icone)."""
        C = self.cores
        ff = C["font_family"]

        wrapper = ctk.CTkFrame(self.sidebar_scroll, fg_color="transparent", height=38)
        wrapper.pack(fill="x", padx=10, pady=2)
        wrapper.pack_propagate(False)

        # Indicador vertical (barra de acento ativa)
        indicador = ctk.CTkFrame(wrapper, width=3, fg_color="transparent", corner_radius=2)
        indicador.pack(side="left", fill="y", padx=(0, 6))

        botao = ctk.CTkButton(
            wrapper, text=f"  {icone}   {texto}",
            height=36, fg_color="transparent", hover_color=C["surface"],
            text_color=C["text_muted"], corner_radius=8, anchor="w",
            font=(ff, 12, "bold"),
        )
        botao.pack(side="left", fill="both", expand=True)
        # anexa metadados para o executar_menu manipular
        botao._cw_indicador = indicador
        botao._cw_cor_icone = cor_icone
    ...
    def executar_menu(self, botao, comando):
        ...
        for b in self.botoes_menu:
            ...
            ind = getattr(b, "_cw_indicador", None)
            if ind is not None:
                ind.configure(fg_color="transparent")
        botao.configure(fg_color=C["brand"], text_color="#FFFFFF")
        ind = getattr(botao, "_cw_indicador", None)
        if ind is not None:
            ind.configure(fg_color=C["brand"])
        comando()

    def ativar_primeiro_botao(self):
        if self.botoes_menu:
            self.executar_menu(self.botoes_menu[0], lambda: None)
