...
from ui.components import CWButton, ButtonVariant, CWCard
...
class TelaPerfil(QWidget):
    ...
    def _build_avatar_section(self):
        ...
        btn_trocar = CWButton("Trocar Foto", ButtonVariant.PRIMARY)
        btn_trocar.clicked.connect(self._trocar_foto)
        btn_col.addWidget(btn_trocar)

        btn_remover = CWButton("Remover Foto", ButtonVariant.GHOST)
    ...
    def _build_nome_section(self):
        ...
        btn_salvar_nome = CWButton("Salvar Alterações", ButtonVariant.PRIMARY)
    ...
    def _build_senha_section(self):
        ...
        btn_alterar_senha = CWButton("Alterar Senha", ButtonVariant.PRIMARY)
        ...
        if perfil_service.save_avatar(self._usuario_id, file_path):
            ...
            if avatar_path:
                ...
                if pix:
                    self._avatar_label.setPixmap(pix)
                    self._avatar_label.setText("")
                    # Notifica o header para atualizar também
                    from utils.avatar import avatar_bus
                    avatar_bus.avatar_updated.emit(self._usuario_id)
    ...
    def _salvar_nome(self):
        ...
        if perfil_service.update_nome(self._usuario_id, novo_nome):
            ...
            # Atualiza avatar se estava sem foto
            avatar_path = perfil_service.get_avatar_path(self._usuario_id)
            if not avatar_path:
                self._set_avatar_initials(novo_nome)
