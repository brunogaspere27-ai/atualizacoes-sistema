...
# ---------------------------- theme ----------------------------
# Paleta Dark Premium - sem dependência de cw_theme
...
def label(value="", size=10, color=TEXT, bold=False):
    w = QLabel(str(value))
...
class DashboardCW(QWidget):
    ...
    def _topbar(self):
        ...
        brand = QVBoxLayout()
        brand.setSpacing(0)
        brand.addWidget(label("CW TRANSPORTADORA", 15, TEXT, True))
        brand.addWidget(label("Executive Analytics", 8, TEXT2))
        row.addLayout(brand)
        ...
        today = label(datetime.now().strftime("%d/%m/%Y"), 9, TEXT2, True)
        row.addWidget(today)
    ...
    def _filters(self):
        ...
        l.addWidget(label("VISÃO", 8, TEXT3, True))
        ...
        l.addWidget(label("ATUALIZADO AGORA",8,TEXT3,True))
    ...
    def _hero(self):
        ...
        left = QVBoxLayout()
        left.setSpacing(3)
        left.addWidget(label("RECEITA BRUTA", 9, TEXT3, True))
        self.hero_value = label("R$ —", 31, TEXT, True)
        left.addWidget(self.hero_value)
        self.hero_delta = label("—", 9, EMERALD)
        left.addWidget(self.hero_delta)
        l.addLayout(left, 2)

        l.addWidget(self._vline())

        self.hero_lucro = self._metric("Lucro estimado", "R$ —", EMERALD)
        self.hero_fretes = self._metric("Fretes realizados", "—", SKY)
        self.hero_andamento = self._metric("Em andamento", "—", AMBER)
    ...
    def _vline(self):
        x = QFrame()
        x.setFixedWidth(1)
        x.setStyleSheet(f"background:{BORDER};border:none;")
        return x
    ...
    def _metric(self, title, value, color):
        ...
        box.setSpacing(4)
        box.addWidget(label(title.upper(), 8, TEXT3, True))
        val = label(value, 18, TEXT, True)
        box.addWidget(val)
        dot = label("●", 8, color)
        box.addWidget(dot)
        box.value = val
    ...
    def _analytics(self):
        ...
        title_box.addWidget(label("Receita x despesas", 14, TEXT, True))
        title_box.addWidget(label("Acompanhe a margem ao longo do período", 9, TEXT2))
        head.addLayout(title_box)
        head.addStretch()
        head.addWidget(StatusDot("Receita", EMERALD))
        head.addSpacing(10)
        head.addWidget(StatusDot("Despesa", ROSE))
        cl.addLayout(head)
        ...
        sl.addWidget(label("Resumo operacional", 14, TEXT, True))
        sl.addWidget(label("Onde a operação está agora", 9, TEXT2))

        self._status_row(sl, "Entregues", EMERALD)
        self._status_row(sl, "Em trânsito", SKY)
        self._status_row(sl, "Pendentes", AMBER)
        ...
        sl.addWidget(label("Utilização", 11, TEXT, True))
        self.util_frota = self._util_row("Frota disponível", 86, SKY)
        self.util_prazo = self._util_row("Entregas no prazo", 92, EMERALD)
    ...
    def _util_row(self, name, pct, color):
        ...
        box.setSpacing(4)
        top = QHBoxLayout()
        top.addWidget(label(name, 9, TEXT2))
        top.addStretch()
        val = label(f"{pct}%", 9, TEXT, True)
        top.addWidget(val)
        box.addLayout(top)
        ...
        bar.setStyleSheet(
            f"""
            QFrame {{
                background:{ELEVATED};border:none;border-radius:3px;
            }}
            """
        )
        # fixed child fill
    ...
    def _operations(self):
        title = QHBoxLayout()
        title.addWidget(label("Operação", 16, TEXT, True))
        title.addWidget(label("  /  visão de campo", 9, TEXT3))
        title.addStretch()
        ...
        tl.addWidget(label("Próximas entregas", 13, TEXT, True))
        ...
        self.table.setStyleSheet(
            ...
            QHeaderView::section {{
                background:{ELEVATED};color:{TEXT3};
        ...
        rl.setSpacing(10)
        rl.addWidget(label("Financeiro", 13, TEXT, True))
        rl.addWidget(label("posição resumida", 9, TEXT2))

        self.fin_receive = self._finance_item(rl,"A receber",EMERALD)
        self.fin_pay = self._finance_item(rl,"A pagar",ROSE)
        self.fin_fuel = self._finance_item(rl,"Combustível",AMBER)
        self.fin_maint = self._finance_item(rl,"Manutenção",SKY)
    ...
    def _finance_item(self, parent, title, color):
        ...
        l.setSpacing(1)
        l.addWidget(label(title,8,TEXT2,True))
        val = label("R$ —",13,TEXT,True)
        det = label("—",8,TEXT3)
        l.addWidget(val)
        l.addWidget(det)
